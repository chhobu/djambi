...
class Faction implements ArrayableInterface {
  ...
  public function setDrawStatus($value) {
    if (is_null($value)) {
      ...
      $this->getBattlefield()->getGameManager()->setStatus(StatusEnum::STATUS_DRAW_PROPOSAL);
    }
  ...
  public function callForADraw() {
    if ($this->getBattlefield()->getGameManager()->getStatus() != StatusEnum::STATUS_PENDING || !$this->canCallForADraw()) {
  ...
  public function acceptDraw() {
    if ($this->getBattlefield()->getGameManager()->getStatus() != StatusEnum::STATUS_DRAW_PROPOSAL &&
